...
export class FlexTreeLayout {
    ...
    receiveTree(tree) {
        const wtree = this.wrap(this.getWrapper(), tree, node => node.children);
        wtree.update();
        return wtree.data;
    }
    getFlexNode() {
        const nodeSize = this.accessor('nodeSize');
        const spacing = this.accessor('spacing');
    ...
    getWrapper() {
        const FlexNode = this.getFlexNode();
        const nodeSize = this.accessor('nodeSize');
        const spacing = this.accessor('spacing');
    ...
    hierarchy(treeData, children) {
        ...
        return this.wrap(this.getFlexNode(), treeData, kids);
    }
    dump(tree) {
        const nodeSize = this.accessor('nodeSize');
        const _dump = i0 => node => {
...
const separate = (w, i, lows) => {
    ...
    while (rContour && lContour) {
        ...
        // the "spacing"
        const dist = (rSumMods + rContour.prelim) - (lSumMods + lContour.prelim) +
