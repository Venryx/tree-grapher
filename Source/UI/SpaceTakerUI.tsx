...
import {Graph, useForceUpdate} from "../index.js";

export function useRef_spaceTakerUI(graph: Graph, handle: SpaceTakerUI_Handle) {
	...
	const ref_spaceTakerUI = useCallbackRef<HTMLDivElement>(null, el=>{
		if (el) {
			handle.divEl = el;
...
export class SpaceTakerUI_Handle {
	...
	divEl: HTMLDivElement; // maybe rework this
	forceUpdate: ()=>void;
}

export const SpaceTakerUI = React.memo((props: {graph: Graph, scaling?: number})=>{
	const forceUpdate = useForceUpdate();

	const handle = useMemo(()=>new SpaceTakerUI_Handle({props: props as any, divEl: null as any, forceUpdate}),
